...
export const CustomerDetail = ({ id }: { id: string }) => {
  ...
  const activity = React.useMemo(() => {
    const t = (txns?.results ?? []).map((x) => ({
      kind: "txn" as const,
      date: x.createdAt,
      data: x as Transaction,
    }));
    const o = (orders?.results ?? []).map((x) => ({ kind: "order" as const, date: x.createdAt, data: x as Order }));
    return [...t, ...o].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 20);
  }, [txns, orders]);
    ...
    <div>
      ...
      <div className="mt-4 rounded-2xl border bg-card p-6">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          ...
          <CustomerActions userId={id} status={user?.status} orders={recentOrders?.results ?? []} currency={currency} />
        </div>
      </div>
          ...
          <div className="space-y-4">
            {user?.lastLoginDate && (
              <ActivityRow
                color="#0284C7"
                title="Last login"
                subtitle={dayjs(user.lastLoginDate).format("DD MMM YYYY, HH:mm")}
              />
            )}
            {activity.length ? (
              activity.map((item, i) =>
