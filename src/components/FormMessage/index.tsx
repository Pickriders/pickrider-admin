import React, { ChangeEventHandler } from "react";
import { TextInputWithCounter } from "../ui/InputTextCounter";
import { TextAreaWithCounter } from "../ui/TextAreaWithCounter";
import { SVG } from "../svg";
import { UI } from "../ui";

interface MessageFormProps {
  title: string;
  message: string;
  attachment?: Boolean;
  onTitleChange: React.Dispatch<React.SetStateAction<string>>;
  onMessageChange: React.Dispatch<React.SetStateAction<string>>;
  onAttachmentChange?: (file: File | undefined) => void;
  onSubmit?: () => void;
  onCancel?: () => void;
}

export const FormMessage = ({
  onSubmit,
  message,
  onMessageChange,
  title,
  onTitleChange,
  attachment,
  onAttachmentChange,
}: MessageFormProps) => {
  const handleAttachmentChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const file = event.target.files?.[0];
    if (onAttachmentChange) {
      onAttachmentChange(file);
    }
  };

  return (
    <form className="mt-10  space-y-8">
      <TextInputWithCounter setValue={onTitleChange} value={title} />
      <TextAreaWithCounter setValue={onMessageChange} value={message} />
      {attachment && (
        <label
          htmlFor="attachment"
          className="w-full gap-x-2 cursor-pointer border-[1.5px] text-muted-foreground px-2 text-sm flex justify-center items-center font-montserrat font-semibold py-2.5 rounded-[4px] bg-muted dark:bg-transparent border-[#C7CBE0] dark:border-neutral-800 hover:bg-gray-200 dark:hover:bg-transparent"
        >
          <SVG.LinkIcon />
          Add Attachment
          <input
            type="file"
            id="attachment"
            className="hidden"
            onChange={handleAttachmentChange}
          />
        </label>
      )}

      <div className="flex flex-col gap-y-3">
        <UI.PrimaryButton>Send</UI.PrimaryButton>
        <UI.PrimaryButton type="button" variant={"outline"}>
          Cancel
        </UI.PrimaryButton>
      </div>
    </form>
  );
};
