export const ShieldUser = () => {
  return (
    <svg
      width="24"
      height="25"
      viewBox="0 0 24 25"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M14 9C14 10.1045 13.1046 11 12 11C10.8954 11 10 10.1045 10 9C10 7.89543 10.8954 7 12 7C13.1046 7 14 7.89543 14 9Z"
        stroke="white"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M9.59003 14.1482C8.96125 14.5167 7.31261 15.2693 8.31674 16.211C8.80725 16.671 9.35355 17 10.0404 17H13.9596C14.6464 17 15.1928 16.671 15.6833 16.211C16.6874 15.2693 15.0388 14.5167 14.41 14.1482C12.9355 13.2839 11.0645 13.2839 9.59003 14.1482Z"
        stroke="white"
        strokeWidth="1.5"
        strokeLinejoin="round"
      />
      <path
        d="M21 11.6833V8.78029C21 7.14029 21 6.32028 20.5959 5.78529C20.1918 5.25029 19.2781 4.99056 17.4507 4.4711C16.2022 4.1162 15.1016 3.68863 14.2223 3.29829C13.0234 2.7661 12.424 2.5 12 2.5C11.576 2.5 10.9766 2.7661 9.77771 3.29829C8.89839 3.68863 7.79784 4.11619 6.54933 4.4711C4.72193 4.99056 3.80822 5.25029 3.40411 5.78529C3 6.32028 3 7.14029 3 8.78029V11.6833C3 17.3085 8.06277 20.6835 10.594 22.0194C11.2011 22.3398 11.5046 22.5 12 22.5C12.4954 22.5 12.7989 22.3398 13.406 22.0194C15.9372 20.6835 21 17.3085 21 11.6833Z"
        stroke="white"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );

};
